...
app.post("/admin/roles", async (c) => {
  ...
  if (!name) return c.json({ error: "Designation name is required" }, 400);

  const existing = await c.env.DB.prepare("SELECT id FROM roles WHERE name = ?").bind(name).first();
  if (existing) return c.json({ error: "A designation with that name already exists" }, 400);
...
app.patch("/admin/roles/:id", async (c) => {
  ...
  if (!existing) return c.json({ error: "Designation not found" }, 404);
  ...
  if (dupe) return c.json({ error: "A designation with that name already exists" }, 400);
